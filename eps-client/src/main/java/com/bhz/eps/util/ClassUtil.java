...
public class ClassUtil {
	...
	public static void initTypeToMsgClassMap() throws ClassNotFoundException, IOException {
		...
		if (classSet != null) {
			for (Class<?> clazz : classSet) {
				if (clazz.isAnnotationPresent(BizProcessorSpec.class)) {
					BizProcessorSpec annotation = clazz.getAnnotation(BizProcessorSpec.class);
	...
	/**
	 * 初始化typeToProcessorClassMap
	 * 遍历包com.bhz.eps.processor
	 * 取得业务逻辑执行器的class文件
	 * 
	 * @throws ClassNotFoundException
	 * @throws IOException
...
class MyFilter implements FileFilter {
	...
	public boolean accept(File file) {
		return (recursive && file.isDirectory()) || (file.getName().endsWith(".class") && file.getName().contains("$") == false && file.getName().endsWith("Processor.class"));
	}
}